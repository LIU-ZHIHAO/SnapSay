...
export function learnFromCorrections(
  ...
  const updatedWordbook = JSON.parse(JSON.stringify(currentWordbook)) as WordbookEntry[];
  let addedCount = 0;
  let updatedCount = 0;

  // 筛选出有用户纠错，且更新时间晚于 learnedAt 或是新记录的已完成记录
  const eligibleRecords = records.filter((r) => {
    if (r.status !== 'completed') return false;
    if (!r.userCorrection || !r.userCorrection.trim()) return false;
    if (!r.transcript || !r.transcript.trim()) return false;
    // 如果用户纠错和原始转写完全一样，没有纠错发生，跳过
    if (r.userCorrection.trim() === r.transcript.trim()) return false;

    if (learnedAt) {
      const recordTime = r.updatedAt || r.createdAt;
      if (!recordTime || recordTime <= learnedAt) {
        return false;
      }
    }
    return true;
  ...
  for (const record of eligibleRecords) {
    const candidates = extractWordPairCandidates(record.transcript, record.userCorrection!);
    for (const cand of candidates) {
      const targetWord = cand.to.trim();
      const variantWord = cand.from.trim();
      if (!targetWord || !variantWord) continue;

      // 查找词库中是否已存在该 targetWord 词条
      const existingEntry = updatedWordbook.find(
        (entry) => entry.target.toLowerCase() === targetWord.toLowerCase()
      );

      if (existingEntry) {
        // 如果 targetWord 存在，检查 variantWord 是否已在 variants 中
        const hasVariant = existingEntry.variants.some(
          (v) => v.toLowerCase() === variantWord.toLowerCase()
        );
        // 变体不能和 targetWord 本身相同（忽略大小写）
        const isSameAsTarget = existingEntry.target.toLowerCase() === variantWord.toLowerCase();

        if (!hasVariant && !isSameAsTarget) {
          existingEntry.variants.push(variantWord);
          updatedCount++;
        }
      } else {
        // 如果不存在，创建新词条
        ...
        const entryVariants = isSameAsTarget ? [] : [variantWord];
        
        updatedWordbook.push({
          id: generateId(),
          target: targetWord,
