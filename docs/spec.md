# SnapSay Windows 语音输入助手产品规格

## 1. 产品定位

SnapSay 是一个 Windows 桌面语音输入助手，用于替代网络依赖强、订阅成本高、偶发不可用的语音输入工具。第一版目标是完成从“快捷键或鼠标触发录音”到“语音识别、可选 API 文案整理、粘贴到当前光标位置、保存记录兜底”的完整闭环。
...
当前产品版本地 ASR 只保留 `SenseVoiceSmall`：

- 默认模型：`iic/SenseVoiceSmall`，本地路径 `models\sensevoice\SenseVoiceSmall`。
- 当前本地模型元数据：`model_revision=master`，下载元数据创建时间 `2024-09-26 11:36:27 +08:00`。
- 许可证：Apache 2.0。
- 定位：中文优先、低延迟、本地离线转写。
- 当前不在设置页暴露其他本地 ASR 引擎。

FFmpeg 仅作为内部兼容兜底：非 WAV 音频需要转换时尝试调用系统 PATH 中的 `ffmpeg`。项目不内置 FFmpeg，不提供配置入口，不提供安装引导；系统有则可用，没有则按当前音频路径执行并由运行结果决定。

备用方案仅写入开发记录，当前代码不实现：

- `faster-whisper`：可作为后续英文、多语种、批量长音频方案评估。
- `whisper.cpp`：可作为后续轻量 C/C++ 部署方案评估。

后续如果继续优化 SenseVoice，可评估官方或社区实现：`SenseVoiceSmall-onnx`、`SenseVoice.cpp`、`streaming-sensevoice`、`OmniSenseVoice`。当前产品版不接入。

加速策略默认必须是 `GPU 优先`。如果检测或配置不可用，才允许降级到 CPU。

项目提供 `scripts/setup-python-asr.ps1` 用于创建项目内 `.venv`、安装 `FunASR`、CUDA 版 PyTorch，并下载 `iic/SenseVoiceSmall` 到项目 `models` 目录。项目提供 `scripts/test-local-asr.ps1` 对 SenseVoice 做冒烟测试。
