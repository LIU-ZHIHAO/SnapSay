...
- `ffmpeg.exe` 路径，用于将浏览器录音格式转换为 whisper.cpp 更稳定支持的 wav。

默认路径必须位于项目或 D 盘目录，例如 `D:\Antigravity\tailkall\models\whisper\...`。如果本地可执行文件、模型或 ffmpeg 缺失，软件必须在语音记录中保存失败原因，而不是丢弃录音流程。
...
项目提供 `scripts/setup-local-asr.ps1` 用于下载 whisper.cpp CUDA/CPU 包、`ggml-small.bin` 模型，并把 ffmpeg 复制到项目 `models\whisper` 目录。下载的模型和二进制文件禁止提交到 git。
