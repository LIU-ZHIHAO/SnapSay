# TailKall Windows 语音输入助手产品规格

## 1. 产品定位

TailKall 是一个 Windows 桌面语音输入助手，用于替代网络依赖强、订阅成本高、偶发不可用的语音输入工具。第一版目标是完成从“快捷键或鼠标触发录音”到“语音识别、可选 API 文案整理、粘贴到当前光标位置、保存记录兜底”的完整闭环。

软件必须是一个正常的 Windows 桌面应用，有主窗口用于配置模型、快捷键、鼠标按键、提示样式、输出方式和历史记录。软件后续可以支持托盘，但第一版不能只做成托盘程序。

## 2. MVP 范围

第一版包含以下能力：

- 支持全局触发配置：键盘组合键、键盘单键、鼠标中键、鼠标侧键。
- 支持两种录音模式：点击切换、长按录音。
- 支持屏幕正中下方的绿色悬浮录音提示条。
- 支持本地语音识别和云端语音识别。
- 支持 API 文案整理，不做本地 LLM 文案整理。
- 支持自动粘贴到当前光标所在位置。
- 支持语音输入记录，保存原始识别文本、整理后文本、状态、模型信息和复制操作。
- 支持设置页：ASR 服务、文案整理 API、Prompt 模板、快捷键、鼠标按键、存储路径和输出行为。

## 3. 触发系统

触发系统需要支持：

- 键盘组合键，例如 `Ctrl + Alt + V`。
- 键盘单键，例如 `F8`、`F9`、`Pause`。
- 鼠标中键。
- 鼠标侧键，例如 Mouse4、Mouse5。

第一版只需要一个主触发键。后续版本可以支持多个触发键。

如果用户绑定了高冲突按键，例如普通字母、数字、空格、回车、Esc 或常见编辑快捷键，界面必须给出风险提醒。功能上可以允许用户确认后继续使用，但不能静默接受。

## 4. 录音模式

软件支持两种录音模式：

- 点击切换：按一下开始录音，再按一下结束录音并进入处理流程。
- 长按录音：按下开始录音，松开结束录音并进入处理流程。

录音模式可以在设置页切换。录音一旦开始，悬浮提示条必须立即显示。

## 5. 悬浮录音提示条

悬浮录音提示条固定显示在屏幕正中下方，视觉为绿色胶囊形态。

状态包括：

- 录音中：显示 `语音输入` 和实时音量波形动画。
- 识别中：显示 `识别中` 和轻量 loading。
- 整理中：显示 `整理中` 和轻量 loading。
- 已输入：短暂显示 `已输入` 后隐藏。
- 已保存：短暂显示 `已保存` 后隐藏。
- 失败：短暂显示 `输入失败，已保存` 后隐藏。

提示条必须显示在普通窗口上方，但不能抢占当前输入焦点。

## 6. 语音识别

第一版支持两类 ASR：

- 本地 ASR：用于离线、隐私友好、网络不可用时的语音识别。
- 云端 ASR：用于接入豆包或其他语音识别 API。

当前本地 ASR 第一版采用 `whisper.cpp` CLI 适配器。用户需要在设置页配置：

- `whisper-cli.exe` 路径。
- Whisper 模型文件路径，例如 `ggml-small.bin`。
- 系统 `ffmpeg` 命令，用于非 WAV 录音格式的兜底转换；默认不再随项目内置 FFmpeg。

默认路径必须位于项目或 D 盘目录，例如 `D:\Antigravity\tailkall\models\whisper\...`；FFmpeg 兜底只允许调用系统 PATH 中的 `ffmpeg` 或用户显式配置的路径。如果本地可执行文件、模型或 ffmpeg 缺失，软件必须在语音记录中保存失败原因，而不是丢弃录音流程。

本地 ASR 引擎规划为三档：

- `SenseVoice / FunASR`：中文优先，适合普通话、粤语、多语言混合，是后续默认中文高质量方案。
- `faster-whisper`：工程稳定，适合 GPU/CPU 量化部署和长期产品化。
- `whisper.cpp`：当前已接入，依赖少，适合先完成离线录音转文字闭环。

加速策略默认必须是 `GPU 优先`。如果检测或配置不可用，才允许降级到 CPU。

项目提供 `scripts/setup-local-asr.ps1` 用于下载 whisper.cpp CUDA/CPU 包与 `ggml-small.bin` 模型；脚本不再下载或复制 FFmpeg 到项目目录。下载的模型和二进制文件禁止提交到 git。

项目提供 `scripts/setup-python-asr.ps1` 用于创建项目内 `.venv`、安装 `faster-whisper`、`FunASR`、CUDA 版 PyTorch，并下载 `Systran/faster-whisper-small` 与 `iic/SenseVoiceSmall` 到项目 `models` 目录。项目提供 `scripts/test-local-asr.ps1` 对 `whisper.cpp`、`faster-whisper`、`SenseVoice / FunASR` 三套本地 ASR 做冒烟测试。

推荐本地 ASR 候选：

- SenseVoice 或 FunASR：中文优先。
- whisper.cpp 或 faster-whisper：成熟通用 fallback。

供应商配置不能硬编码。用户必须能在桌面软件中配置提供商类型、接口地址或本地模型路径、模型名称和相关选项。

大模型文件、缓存、日志、录音、临时音频片段和运行数据不能默认写入 C 盘用户目录或系统盘。默认目录应优先放在项目或应用数据目录下的 `D:\Antigravity`、`D:\SoftInstall`、项目内目录，或用户明确选择的非 C 盘目录。

## 7. 文案整理

MVP 的文案整理只支持 API，不做本地 LLM 整理。

支持的整理模式：

- 不整理：ASR 识别完成后直接粘贴原文。
- API 整理：将识别文本发送给配置好的模型提供商，例如 DeepSeek 或其他 OpenAI-compatible API。

API 配置必须支持：

- 提供商预设。
- 显示名称。
- Base URL。
- API Key。
- 模型名称。
- 测试连接按钮。
- Prompt 模板编辑器。

API Key 禁止硬编码、禁止提交、禁止完整写入日志。界面和日志最多只能展示脱敏后的 Key。

Prompt 模板必须可编辑并保存为配置，不能散落硬编码在业务逻辑中。

## 8. 输入流水线

正常处理流程：

```text
检测到触发键
  -> 开始录音
  -> 结束录音
  -> 执行 ASR
  -> 可选 API 文案整理
  -> 粘贴到当前光标位置
  -> 保存语音输入记录
```

粘贴策略：

- 第一版优先使用剪贴板粘贴，兼容性最高。
- 软件将最终文本写入剪贴板，模拟粘贴快捷键，并记录插入状态。
- 在可行时恢复用户原剪贴板内容，但用户文本成功插入优先级更高。

如果光标丢失、目标软件阻止粘贴、权限不足或插入失败，软件仍必须保存记录，用户可以回到主窗口手动复制。

## 9. 语音记录

主窗口包含语音记录列表。每条记录包含：

- 创建时间。
- 原始识别文本。
- 整理后文本。
- ASR 提供商和模型。
- 文案整理提供商和模型。
- 音频时长。
- 状态：已输入、仅保存、失败、已取消。
- 失败原因。

记录操作：

- 复制原始识别文本。
- 复制整理后文本。
- 使用当前 API 配置重新整理。
- 再次粘贴到当前光标。
- 删除记录。

表格或列表中的长文本默认必须精简显示，禁止溢出布局。鼠标悬停需要能查看完整内容。如果字段可编辑，点击应支持就地修改。

## 10. 主窗口结构

桌面应用包含：

- 主页：显示当前状态、ASR 提供商、整理 API、触发键和最近输入记录。
- 语音记录：可搜索的记录列表，支持复制、重试和删除。
- 设置页：触发键、录音模式、悬浮提示、粘贴行为、模型供应商、Prompt 和存储路径。

应用必须可以作为普通窗口使用，后续可以补充最小化到托盘。

## 11. 设置要求

设置页分区：

- 触发方式：键盘或鼠标按键捕获、冲突提醒、录音模式。
- ASR 提供商：本地 ASR 和云端 ASR 配置。
- 文案整理 API：OpenAI-compatible 提供商配置和连接测试。
- Prompt 模板：可编辑的整理 Prompt。
- 输出方式：直接输入、整理后输入、仅保存。
- 存储路径：应用数据、日志、缓存、模型和临时音频目录。

所有数值设置项必须仅支持手动输入数字 `0-9`。数值输入框禁止浏览器或系统原生上下按钮。

## 12. 数据和存储规则

运行数据禁止默认写入 C 盘用户目录或系统盘，包括：

- 日志。
- 数据库。
- 音频录音。
- 临时音频片段。
- 缓存文件。
- 模型文件。
- 下载中间文件。

只有极小型配置文件，并且不包含日志、数据库、大缓存、模型或媒体文件时，才可以按系统规范放到用户配置目录。如果用户明确选择了目录，软件必须尊重用户选择。

## 13. 错误处理

必须处理以下错误：

- 触发键冲突。
- 麦克风不可用。
- ASR 提供商不可用。
- 本地模型缺失。
- API 提供商不可用。
- API Key 缺失或无效。
- 文案整理超时。
- 剪贴板或粘贴失败。
- 光标目标丢失。

只要已经有识别文本或最终文本，即使输入失败，也必须保存语音记录。

## 14. 开发阶段

阶段 1：产品壳和 UI 原型。

- 主窗口。
- 设置页。
- 语音记录列表。
- 悬浮提示条。

阶段 2：触发和录音。

- 全局键盘绑定。
- 鼠标中键和侧键绑定。
- 点击切换和长按录音。
- 麦克风采集。

阶段 3：识别和整理。

- 本地 ASR 接口。
- 云端 ASR 接口。
- OpenAI-compatible 文案整理接口。
- DeepSeek 预设。
- 连接测试。

阶段 4：插入和记录。

- 剪贴板粘贴。
- 失败兜底。
- 记录持久化。
- 复制和重试。

阶段 5：加固。

- 提供商超时。
- 存储路径校验。
- 触发冲突提醒。
- 打包。
- Windows 兼容性人工测试。
